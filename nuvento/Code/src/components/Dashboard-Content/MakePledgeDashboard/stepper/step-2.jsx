...
export default function StepTwo() {
  ...
  const pledgeAmount = localStorage.getItem("PledgeAmtD");
  const FundingProgram = localStorage.getItem("FundingProgramD");
  const ApplyTo = localStorage.getItem("ApplyToD");
  ...
  const handleChange = event => {
    ...
    localStorage.setItem("FundingProgramD", event.target.value);
  };

  const handleChangeApplyTo = event => {
    
    setApplyTo(event.target.value);
    localStorage.setItem("ApplyToD", event.target.value);
  };
  ...
  const handleValidataionChange = (event) => {
    ...
    if (event.target.name == "pledgeAmount") {
      if (event.target.value.IsCurrency() || event.target.value == "") {
        ...
        localStorage.setItem("PledgeAmtD", event.target.value);
      }
    }
  };
    ...
    <div className='rows'>
      <div class="steptwo">
        <div className='accountarea accountareastep2'>
          <div id="addPledgeSection2" class="grouppay commontexrmob">
            <TextField
              ...
              variant="filled"
              inputProps={{
                'aria-label':'Enter pledge amount',
            ...
            />
            {/* <TextField required id="filled-required" value="$0.00" label="Pledge Amount" {...css(commfiled)} className={'commontextfield'} margin="normal" variant="filled" /> */}
    ...
    <option value={m}>{m}</option>
            ...
            {/* <FormControl variant="filled" className={classes.formControlselect}>
              ...
              >
                ...
                <MenuItem value="Letter of Deposit">Letter of Deposit</MenuItem>
              </Select>
            </FormControl> */}
