...
export default function EnhancedTable(props) {
    ...
    const [search, setSearch] = useState(""); /// created by me
    const [parentData, setParentData] = useState([]); /// created by me
    ...
    const searchbar = (e) =>{
        ...
        const userList = parentData && parentData.length >= 1 ? parentData.filter(datas => {
            return (
            
                (datas.Customer && datas.Customer.toLowerCase().includes(search))||
                (datas.Account && datas.Account.toLowerCase().includes(search))||
                (datas.PledgeValue && datas.PledgeValue.toLowerCase().includes(search))||
                (datas.PledgeDate && datas.PledgeDate.toLowerCase().includes(search))||
                (datas.ExpiryDate && datas.ExpiryDate.toLowerCase().includes(search))||
                (datas.Status && datas.Status.toLowerCase().includes(search))
